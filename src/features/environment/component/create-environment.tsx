...
const CreateEnvironment = ({
  ...
  const { createEnvironemnt, pending, fail } = useCreateEnvironment();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await createEnvironemnt(formData);
      await onRefresh();
      showToast("Environment créé avec succès", "success");
      onClose();
      setForm({ id: 0, titre: "", resume: "", photo: null as File | null });
    } catch (err) {
      console.error(err);
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
        <div className="flex justify-between items-center p-5 border-b">
          <h2 className="text-xl font-bold text-gray-800">Environnemnt</h2>
