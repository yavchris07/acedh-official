...
import { useUpdateAdress } from "../hooks/use-update-adress";
import { useToast } from "../../../components/customer-toast";
import type { Adress } from "../../../utils/type";

type updateAdressProps = {
  open: boolean;
  onClose: () => void;
  adress: Adress;
  onRefresh: () => void;
...
const UpdateAdress = ({
  ...
  const { updateAdress, load, er } = useUpdateAdress();
  const { showToast } = useToast();

  const handleSubmit = async (e: React.FormEvent<HTMLElement>) => {
    e.preventDefault();
    try {
      await updateAdress(adress.id, formData);
