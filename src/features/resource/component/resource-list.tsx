import { Edit2Icon, FileText, Trash2Icon } from "lucide-react";
import type { Resource } from "../../../utils/type";

type resourceListProps = {
  resources: Resource[];
  loading: boolean;
  fail: string;
  onEdit: (partner: Resource) => void;
  onDelete: (partner: Resource) => void;
};

const ResourceList = ({
  onDelete,
  onEdit,
  resources,
  fail,
  loading,
}: resourceListProps) => {
  return (
    <div className="bg-white w-full rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      <table className="w-full text-left">
        <thead className="bg-gray-50 border-b">
          <tr>
            <th className="px-6 py-4 text-sm font-semibold text-gray-600">
              Titre
            </th>
            <th className="px-6 py-4 text-sm font-semibold text-gray-600">
              Fichier
            </th>
            <th className="px-6 py-4 text-sm font-semibold text-gray-600">
              Actions
            </th>
          </tr>
        </thead>
        <tbody className="divide-y">
          {loading && (
            <tr className="text-green-500 text-sm text-center">
              <td> Chargement ...</td>
            </tr>
          )}
          {fail && (
            <tr>
              <td className="text-red-500 text-sm text-center">
                Erreur due a l&apos;absence de la connexion internet !
              </td>
            </tr>
          )}
          {resources.map((resource, i: number) => (
            <tr key={i} className="hover:bg-gray-50 transition">
              <td className="px-6 py-4 text-gray-500 font-medium">
                {resource.titre.substring(0, 57)}...
              </td>
              <td className="px-6 py-4 text-gray-500 font-medium">
                {resource.fichier_pdf ? (
                  <FileText color="green" />
                ) : (
                  "Erreur fichier n'est pas present"
                )}
              </td>
              <td className="px-6 py-4 flex items-center gap-4">
                <button
                  onClick={() => onEdit(resource)}
                  className="text-blue-500 hover:text-blue-300"
                >
                  <Edit2Icon
                    size={18}
                    className="text-blue-500 hover:text-blue-300"
                  />
                </button>
                <button
                  onClick={() => onDelete(resource)}
                  className="text-red-400 hover:text-red-300"
                >
                  <Trash2Icon size={18} />
                </button>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
};

export default ResourceList;
