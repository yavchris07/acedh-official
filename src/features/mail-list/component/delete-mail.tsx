...
import { useDeleteMail } from "../hooks/use-delete-mail-list";
import { X, Loader2, Trash2Icon } from "lucide-react";
import { useToast } from "../../../components/customer-toast";
import type { Mail } from "../../../utils/type";

type deleteMailProps = {
  open: boolean;
  onClose: () => void;
  mail: Mail;
};
const DeleteMail = ({ open, mail, onClose }: deleteMailProps) => {
  const { showToast } = useToast();
  const { deleteMail, pending, error } = useDeleteMail();
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await deleteMail(mail.id);
      // alert("Membre supprimé avec succès !");
      showToast("Email supprimé avec succès !", "error");
      onClose();
    } catch (err) {
      console.error(err);
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
        <div className="flex justify-between items-center p-5 border-b">
          <h2 className="text-xl font-bold text-gray-800">Supprimer le mail</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="text-gray-500">
            Êtes-vous sûr de vouloir supprimer cette adresse mail{" "}
